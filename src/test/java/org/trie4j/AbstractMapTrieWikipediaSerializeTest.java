...
public abstract class AbstractMapTrieWikipediaSerializeTest extends AbstractWikipediaSerializeTest {
    ...
    public void test() throws Exception {
        ...
        lt.reset();
        MapTrie<Integer> t =
                (MapTrie<Integer>) new ObjectInputStream(new ByteArrayInputStream(serialized)).readObject();
    ...
    private static String getTailClassName(MapTrie<?> trie) {
        if (trie instanceof MapTailPatriciaTrie) {
            return "("
                    + ((MapTailPatriciaTrie) trie).getTailBuilder().getClass().getSimpleName() + ")";
