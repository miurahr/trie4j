...
public class TailLOUDSTrieWithSuffixTrieTailArrayWikipediaMonitoredSBVTest extends AbstractWikipediaTest {
    ...
    protected Trie buildSecondTrie(Trie first) {
        ...
        TailLOUDSTrie t = new TailLOUDSTrie(
                first, new LOUDSBvTree(bv), new SuffixTrieTailArray(first.size()), new NodeListener() {
                    @Override
    ...
    protected void afterVerification(Trie trie) throws Exception {
        System.out.println(String.format(
                ...
                bv.getSelect0Time() / 1000000.0, bv.getSelect0MinTime(), bv.getSelect0MaxTime(), bv.getSelect0Count()));
        System.out.println(
                String.format("next0 time(ms): %.3f, count: %d", bv.getNext0Time() / 1000000.0, bv.getNext0Count()));
        System.out.println(
                String.format("rank1 time(ms): %.3f, count: %d", bv.getRank1Time() / 1000000.0, bv.getRank1Count()));
