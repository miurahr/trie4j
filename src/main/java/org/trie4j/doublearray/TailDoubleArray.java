...
public class TailDoubleArray extends AbstractTermIdTrie implements TermIdTrie, Externalizable {
    ...
    private class TailDoubleArrayNode implements TermIdNode {
        ...
        public char[] getLetters() {
            ...
            TailUtil.appendChars(tailArray.newIterator(tailArray.getIteratorOffset(nodeId)), ret);
    ...
    public Iterable<Pair<String, Integer>> commonPrefixSearchWithTermId(String query) {
        ...
        for (; ci < charsLen; ci++) {
            ...
            if (term.get(ni)) ret.add(Pair.create(query.substring(0, ci + 1), term.rank1(ni) - 1));
        }
        return ret;
    }
    ...
    private void build(
            Node node, int nodeIndex, TailArrayBuilder tailArrayBuilder, FastBitSet bs, TermNodeListener listener) {
        // letters
        ...
        Map<Integer, List<Pair<Node, Integer>>> nodes =
                new TreeMap<Integer, List<Pair<Node, Integer>>>(new Comparator<Integer>() {
                    @Override
        ...
        for (Map.Entry<Integer, List<Pair<Node, Integer>>> e : nodes.entrySet()) {
            for (Pair<Node, Integer> e2 : e.getValue()) {
                build(e2.getFirst(), e2.getSecond() + offset, tailArrayBuilder, bs, listener);
            }
        }
        // */
    }
