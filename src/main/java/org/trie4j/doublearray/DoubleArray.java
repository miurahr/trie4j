...
public class DoubleArray extends AbstractTermIdTrie implements Externalizable, TermIdTrie {
    ...
    public Iterable<Pair<String, Integer>> commonPrefixSearchWithTermId(String query) {
        ...
        for (int i = 0; i < charsLen; i++) {
            ...
            nodeIndex = next;
            if (term.get(nodeIndex)) {
                ret.add(Pair.create(new String(chars, 0, i + 1), term.rank1(nodeIndex) - 1));
            }
        }
        return ret;
    }
    ...
    public Iterable<Pair<String, Integer>> predictiveSearchWithTermId(String prefix) {
        ...
        while (!q.isEmpty()) {
            ...
            for (char v : this.chars) {
                ...
                String n = new StringBuilder(c).append(v).toString();
                if (term.get(next)) {
                    ret.add(Pair.create(n, term.rank1(next) - 1));
                }
                q.push(Pair.create(next, n));
            }
        }
