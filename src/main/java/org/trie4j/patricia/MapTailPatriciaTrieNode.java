...
public class MapTailPatriciaTrieNode<T> extends org.trie4j.patricia.TailPatriciaTrieNode implements Serializable {
    public MapTailPatriciaTrieNode(
            char firstChar, int tailIndex, boolean terminated, MapTailPatriciaTrieNode<T>[] children) {
        super(firstChar, tailIndex, terminated, children);
    }

    public MapTailPatriciaTrieNode(
            char firstChar, int tailIndex, boolean terminated, MapTailPatriciaTrieNode<T>[] children, T value) {
        super(firstChar, tailIndex, terminated, children);
        this.value = value;
    }
    ...
    public TailPatriciaTrieNode addChild(int index, TailPatriciaTrieNode n) {
        MapTailPatriciaTrieNode<T>[] newc = (MapTailPatriciaTrieNode<T>[])
                Array.newInstance(MapTailPatriciaTrieNode.class, getChildren().length + 1);
