...
public class MapTailPatriciaTrie<T> extends TailPatriciaTrie implements MapTrie<T> {
    ...
    public MapNode<T> getRoot() {
        return new MapTailPatriciaTrieNodeAdapter<T>(
                (MapTailPatriciaTrieNode<T>) ((TailPatriciaTrieNodeAdapter) super.getRoot()).getNode(), getTails());
    }

    @Override
    @SuppressWarnings("unchecked")
    public T insert(String text, T value) {
        MapTailPatriciaTrieNode<T> node =
                (MapTailPatriciaTrieNode<T>) insert(((TailPatriciaTrieNodeAdapter) super.getRoot()).getNode(), text, 0);
        T ret = node.getValue();
        node.setValue(value);
        return ret;
    }
    ...
    protected MapTailPatriciaTrieNode<T> newNode(char firstChar, int tailIndex, TailPatriciaTrieNode source) {
        return new MapTailPatriciaTrieNode<T>(
                firstChar,
                tailIndex,
                source.isTerminate(),
    ...
    protected MapTailPatriciaTrieNode<T>[] newNodeArray(TailPatriciaTrieNode... nodes) {
        MapTailPatriciaTrieNode<T>[] ret =
                (MapTailPatriciaTrieNode<T>[]) Array.newInstance(MapTailPatriciaTrieNode.class, nodes.length);
    ...
    private class IterableAdapter
            extends org.trie4j.util.IterableAdapter<Pair<String, TailPatriciaTrieNode>, Map.Entry<String, T>> {
