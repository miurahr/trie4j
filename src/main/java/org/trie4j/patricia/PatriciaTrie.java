...
public class PatriciaTrie extends AbstractTrie implements Serializable, Trie {
    ...
    protected PatriciaTrieNode insert(PatriciaTrieNode node, String letters, int offset) {
        ...
        while (true) {
            ...
            if (i != n) {
                PatriciaTrieNode child1 =
                        newNode(Arrays.copyOfRange(node.getLetters(), i, node.getLetters().length), node);
                PatriciaTrieNode child2 = newNode(letters.substring(i + offset).toCharArray(), true);
                node.setLetters(Arrays.copyOfRange(node.getLetters(), 0, i));
                node.setTerminate(false);
                node.setChildren(
                        (child1.getLetters()[0] < child2.getLetters()[0])
                                ? newNodeArray(child1, child2)
                                : newNodeArray(child2, child1));
                size++;
                nodeSize += 2;
                return child2;
                ...
                PatriciaTrieNode newChild =
                        newNode(Arrays.copyOfRange(node.getLetters(), lettersRest, thisLettersLength), node);
                node.setLetters(Arrays.copyOfRange(node.getLetters(), 0, i));
                node.setTerminate(true);
    ...
    private static void enumLettersWithNode(
            PatriciaTrieNode node, String prefix, List<Pair<String, PatriciaTrieNode>> letters) {
        for (PatriciaTrieNode child : node.getChildren()) {
            String text = prefix + new String(child.getLetters());
