...
public class TailPatriciaTrie extends AbstractTrie implements Serializable, Trie {
    ...
    private void enumLettersWithNode(
            TailPatriciaTrieNode node, String prefix, List<Pair<String, TailPatriciaTrieNode>> letters) {
        TailPatriciaTrieNode[] children = node.getChildren();
        if (children == null) return;
        for (TailPatriciaTrieNode child : children) {
            String text = prefix + new String(child.getLetters(tails));
    ...
    protected TailPatriciaTrieNode insert(TailPatriciaTrieNode node, String letters, int offset) {
        ...
        if (offset == lettersLength) {
            ...
            if (!matchComplete) {
                ...
                node.setChildren(
                        (n1.getFirstLetter() < n2.getFirstLetter()) ? newNodeArray(n1, n2) : newNodeArray(n2, n1));
                size++;
                nodeSize += 2;
                return n2;
            } else {
                ...
                if (child != null) {
                    ...
                    int idx =
                            (offset < lettersLength) ? tailBuilder.insert(letters, offset, lettersLength - offset) : -1;
