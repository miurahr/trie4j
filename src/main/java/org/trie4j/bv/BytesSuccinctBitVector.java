...
public class BytesSuccinctBitVector implements Externalizable, SuccinctBitVector {
    ...
    public BytesSuccinctBitVector(byte[] bytes, int bitsSize) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (zeroCount > 0 && (indexOfIndexBlock != (prevSize0 / BITS0_COUNT_IN_EACH_INDEX))) {
                ...
                indexCache0.set(
                        indexOfIndexBlock,
                        i * 8 + zeroPosInB[zeroPosInB.length - (size0 % BITS0_COUNT_IN_EACH_INDEX) - 1]);
            }
    ...
    private static final int[] MASKS = {0x80, 0xc0, 0xe0, 0xf0, 0xf8, 0xfc, 0xfe, 0xff};
