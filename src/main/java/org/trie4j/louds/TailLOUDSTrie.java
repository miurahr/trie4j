...
public class TailLOUDSTrie extends AbstractTermIdTrie implements Externalizable, TermIdTrie {
    ...
    public TailLOUDSTrie(
            int size, int nodeSize, BvTree bvTree, char[] labels, TailArray tailArray, SuccinctBitVector term) {
        this.size = size;
    ...
    private void build(
            Trie orig, BvTree bvtree, TailArrayBuilder tailArrayBuilder, FastBitSet termBs, NodeListener listener) {
        this.bvtree = bvtree;
        this.size = orig.size();
